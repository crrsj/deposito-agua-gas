...
import br.com.deposito.service.ClienteService;
...
public class ClienteController {
    ...
    @GetMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "endpoint responsável pela busca de cliente pelo id.")
    ...
    @PatchMapping("/{id}")
    @ResponseStatus(HttpStatus.OK)
    @Operation(summary = "endpoint responsável por atualizar dados do cliente.")
    @ApiResponse(responseCode = "200", description = " success", content = {
            @Content(mediaType = "application.json", schema = @Schema(implementation = ResponseEntity.class))
    })
